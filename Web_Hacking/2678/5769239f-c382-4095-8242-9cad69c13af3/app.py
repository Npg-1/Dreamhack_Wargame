...
users = {}          # users라는 이름의 딕셔너리
welcome_feed = []   # welcom_feed라는 이름의 리스트
...
if "admin" not in users:    # 처음에 app.py가 실행될 때 "admin"이 없으면 users에 username = "admin", pw="**REDACTED**", created_at=(현재 시간)으로 해서 생성함
...
# 페이지의 메인 화면으로 GET으로 접속할 때 아래 함수 실행
@app.get("/")   
def index():
    uname = current_user()
...
# 페이지의 메인화면/register 에 GET 또는 POST로 접속할 때 아래 함수 실행
...
# 페이지의 메인화면/login 에 GET 또는 POST로 접속할 때 아래 함수 실행
...
# 페이지의 메인 화면/welcome 으로 GET으로 접속할 때 아래 함수 실행
@app.get("/welcome")
def welcome():
    uname = current_user()
    if not uname:
...
# 페이지의 메인 화면/logout 으로 GET으로 접속할 때 아래 함수 실행
@app.get("/logout")
def logout():
