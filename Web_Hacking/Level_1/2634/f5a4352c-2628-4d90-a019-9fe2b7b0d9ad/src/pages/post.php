  ...
  <div>
    ...
    <div class="card">
      <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:center">
        <span class="pill hot">Includes <?= count($incs) ?></span>  <!-- Include한 개수 입력 -->
        <a class="btn" href="/?p=write"><span class="dot"></span>Write</a>
      </div>
      <div class="hr"></div>

      <?php if (!$incs): ?>   <!-- Include가 없다면 "No Includes"를 띄윔 -->
        <div style="color:rgba(255,255,255,.62);font-size:13px">No includes.</div>
      <?php else: foreach ($incs as $inc): ?> <!-- Include가 있다면 해당 개수만큼 반복함 -->
        <div class="inc">
          <div style="display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;align-items:center">
