...
<body>
  <div class="wrap">
    <div class="card">
      <h1>Just Copy It!</h1>
      <pre class="box" id="charie">{{ charie_text | e }}</pre>
    </div>
  </div>
  <script>
    (function(){
      const box = document.getElementById('charie');
...
def read_charie() -> str:
    path = Path(__file__).with_name("charie.txt")
    return path.read_text(encoding="utf-8")

@app.get("/")
def index():
    return render_template_string(TEMPLATE, charie_text=read_charie())  # TEMPLATE을 HTML로 따로 렌더링해서 띄움
